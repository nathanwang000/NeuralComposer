  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < 72; i++) {
        ...
        ctx.beginPath();
        ctx.strokeStyle = i % 12 === 0 ? gridColors.octave : gridColors.pitch;
        ctx.moveTo(0, y);
        ctx.lineTo(w, y);
        ctx.stroke();
      }
      ...
      events.forEach(({ event, beatOffset, isUser, id }) => {
        ...
        if (isUser) {
          ...
          ctx.fillStyle = isSelected
            ? '#ef4444'
            : (isActive ? `hsla(${hue}, 70%, ${noteBrightness.lightness}%, 1)` : `hsla(${hue}, 55%, ${noteBrightness.altLightness}%, ${noteBrightness.altAlpha})`);
          if (isSelected) {
            ctx.shadowBlur = 20;
